...
$(document).ready(function() {
  ...
  const DATA_MESSAGE = [
    ["Get Healthy with Nike", "./nike.png", "#e95814", 5, 0.25],
    ["End Hunger with Coca-Cola", "./coca cola.png", "#ed1c16", 2, 0.1],
    ["Fight Gender Inequality with Google", "./google.png", "#4285f4", 3, 0.15],
    ["Promote Clean Energy with Amazon", "./amazon.png", "#ff9900", 4, 0.2],
    ["Build Sustainable Cities with Airbnb", "./airbnb.png", "#fd5c63", 6, 0.3]
  ]; // Data message
  ...
  // Unfocus all bars / logos and hide tooltip
  ...
  // Focus ith bar and logo and show tooltip
  function focusBar(barI) {
    // Drop opacity of other bars and logos
    G_BARS.selectAll(".data-bar")
      .filter(function(d, i) {
        return i != barI;
      })
    ...
    // Return this bar and logo to full opacity
    const bar = G_BARS.selectAll(".data-bar").filter(function(d, i) {
      return i == barI;
    });
    ...
    TIP.offset([-120, 0]) // use default offset
      .show(datum, node) // show with default offset
      .offset(getOffset()) // adjust offset based on position with default offset
      .show(datum, node); // show with adjust offset
  }
